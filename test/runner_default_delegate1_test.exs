defmodule HarnaisExampleDefaultDelegateModule1Test do

  @args 0 .. 5
  |> Enum.map(fn ndx -> "arg#{ndx}" |> String.to_atom |> Macro.var(nil) end)
  ...
  |> Enum.map(fn {name, arity} ->
    ...
    signature = @args |> Enum.take(arity)

     def unquote(name)(unquote_splicing(signature)) do
       Map.unquote(name)(unquote_splicing(signature))
     end
...
defmodule HarnaisExampleDefaultDelegateRunner1Test do
  ...
  test "ex_runner_map1: [:b]" do
    ...
    Harnais.run_tests_default_test_value(
      test_module: HarnaisExampleDefaultDelegateModule1Test,
